...
function emptyDraft(columnId = "todo") {
  return { id: uid(), columnId, title: "", description: "", labels: [], subtasks: [], dueDate: "", images: [], createdAt: Date.now() };
}
...
export default function AestheticKanbanBoard() {
  ...
  const [quickTask, setQuickTask] = useState(() => ({ title: "", columnId: "todo", dueDate: dateKey(new Date()) }));
  ...
  function quickAddTask() {
    ...
    if (!title) return;
    setTasks((current) => [{ ...emptyDraft(quickTask.columnId || "todo"), title, dueDate: quickTask.dueDate || dateKey(new Date()), labels: [{ id: uid(), text: "nowe", color: "green" }] }, ...current]);
    setQuickTask((current) => ({ ...current, title: "" }));
  }
  function moveTask(taskId, columnId) {
    setTasks((current) => current.map((task) => (task.id === taskId ? { ...task, columnId } : task)));
  }
  ...
  const dashboardCards = {
    ...
    timeline: <TimelineCard t={t} tasks={timelineTasks} columns={columns} onOpenTask={openTask} onOpenDetails={() => setTimelineOpen(true)} />,
  };
      ...
      <main className="mx-auto max-w-7xl">
        ...
        <AnimatePresence mode="wait" initial={false}>
          {activeSection === "info" ? (
            ...
            <motion.section key="tasks-section" data-task-board className="grid w-full grid-cols-4 items-start gap-3 pb-4 lg:gap-4" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
...
function FontScaleStyles() {
  return <style>{String.raw`
    ...
    @media (max-width: 1180px) {
      ...
      [data-task-board] [data-task-description] { -webkit-line-clamp: 2; }
    ...
    <DashboardCard t={t}>
      ...
      <div className="grid gap-3">
        <input
          ...
          onKeyDown={(event) => { if (event.key === "Enter") onQuickAdd(); }}
        ...
        <div className="grid gap-2">
          {upcomingTasks.slice(0, 3).map((task) => <button type="button" key={task.id} onClick={() => onOpenTask(task)} className={cx("flex items-center gap-2 rounded-2xl px-3 py-2 text-left text-xs font-semibold transition hover:-translate-y-0.5 hover:shadow-sm", t.subtle)}><span className={cx("h-4 w-4 rounded-md border", t.border)} /><span className="min-w-0 flex-1 truncate">{task.title}</span><span className={cx("text-[10px]", t.textSoft)}>{formatDate(task.dueDate)}</span></button>)}
...
function MonthCalendar({ t, isDark, month, tasks, columns = defaultColumns, onOpenTask, onCreateTaskForDate, onPreviousMonth, onNextMonth, onToday }) {
  const days = buildMonthDays(month);
  const tasksByDay = useMemo(() => tasks.reduce((grouped, task) => {
      ...
      <div className="flex h-full min-h-0 flex-col">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-7 grid-rows-[auto_repeat(6,minmax(0,1fr))] gap-x-1.5 gap-y-1.5">
          ...
          {days.map((day) => {
            ...
            const stageDots = dayTasks.slice(0, 4).map((task) => ({ id: task.id, gradient: stageGradientForTask(task, columns), title: stageTitleForTask(task, columns) }));
            ...
            return (
              <div key={day.key} className="flex min-h-0 items-center justify-center">
                <button
                  type="button"
                  onClick={() => firstTask ? onOpenTask(firstTask) : onCreateTaskForDate?.(day.key)}
                  title={firstTask ? `${taskTitlesForTooltip(dayTasks)}
Etap: ${stageTitle}` : `Utwórz nowe zadanie z terminem: ${formatDate(day.key, { day: "2-digit", month: "long", year: "numeric" })}`}
...
function TimelineCard({ t, tasks, columns, onOpenTask, onOpenDetails }) {
    ...
    return (
      <DashboardCard t={t}>
        <div role="button" tabIndex={0} onClick={onOpenDetails} onKeyDown={handleKeyDown} className="flex h-full cursor-pointer flex-col rounded-[1.5rem] outline-none transition focus:ring-4 focus:ring-sky-300/40">
          <DashboardTitle t={t} icon={<Clock3 size={12} />} eyebrow="Plan" title="Tasks Timeline" badge={0} />
          <p className={cx("flex flex-1 items-center rounded-2xl p-4 text-sm", t.subtle)}>Brak tasków z terminami. Kalendarz odpoczywa, podejrzanie spokojnie.</p>
        </div>
      </DashboardCard>
    );
  }

  const taskDates = visibleTasks.map((task) => parseLocalDate(task.dueDate)).filter(Boolean);
  const rawStart = new Date(Math.min(...taskDates.map((date) => date.getTime())));
  const rawEnd = new Date(Math.max(...taskDates.map((date) => date.getTime())));
  rawStart.setDate(rawStart.getDate() - 3);
  rawEnd.setDate(rawEnd.getDate() + 3);

  const rangeMs = Math.max(1, rawEnd.getTime() - rawStart.getTime());
  const percentForDate = (date) => clampNumber(((date.getTime() - rawStart.getTime()) / rangeMs) * 100, 0, 100);
  const axisDays = Array.from({ length: 8 }, (_, index) => {
    const date = new Date(rawStart);
    date.setDate(rawStart.getDate() + index * Math.ceil(rangeMs / 86400000 / 7));
    return date;
  });
  ...
  const showToday = today >= rawStart && today <= rawEnd;

  return (
    <DashboardCard t={t}>
      <div role="button" tabIndex={0} onClick={onOpenDetails} onKeyDown={handleKeyDown} className="flex h-full cursor-pointer flex-col rounded-[1.5rem] outline-none transition focus:ring-4 focus:ring-sky-300/40">
        <DashboardTitle t={t} icon={<Clock3 size={12} />} eyebrow="Plan" title="Tasks Timeline" badge={visibleTasks.length} />
        <div className="relative min-h-[180px] flex-1 overflow-hidden rounded-3xl px-2 pb-10 pt-3">
          <div className="absolute inset-x-2 top-4 bottom-10">
            {[0, 1, 2, 3].map((line) => (
          ...
          {showToday && (
            <div className="absolute bottom-10 top-3 z-20 w-0.5 rounded-full bg-violet-400 shadow-[0_0_14px_rgba(168,85,247,.70)]" style={{ left: `${percentForDate(today)}%` }} title="Dzisiaj">
          ...
          {visibleTasks.map((task, index) => {
            const due = parseLocalDate(task.dueDate);
            const snappedDue = snapDateToDateTicks(due, axisDays) || due;
            const snappedDueKey = snappedDue ? dateKey(snappedDue) : task.dueDate;
            const column = columns.find((item) => item.id === task.columnId);
            const barClass = column?.accent || "from-slate-400 to-slate-600";
            const markerPercent = percentForDate(snappedDue);
            ...
            return (
              <React.Fragment key={task.id}>
                <span
                  ...
                  style={{ left: `${markerPercent}%`, top: `${rowTop + 32}px`, bottom: "34px" }}
                />
                <span
                  ...
                  style={{ left: `${markerPercent}%`, bottom: "34px" }}
                />
                <button
                  type="button"
                  onClick={(event) => {
                  ...
                  title={`${task.title} — ${formatDate(snappedDueKey, { day: "2-digit", month: "2-digit", year: "numeric" })}`}
                >
                  <span className="block truncate text-[11px] font-black leading-tight">{task.title}</span>
                  <span className="block truncate text-[9px] font-bold opacity-85">{taskProgress}% • {formatDate(snappedDueKey)}</span>
                </button>
              </React.Fragment>
            );
          })}
          ...
          <div className="absolute bottom-2 left-0 right-0 z-40 h-9">
            {axisDays.map((date) => {
              ...
              return (
                <div key={tickKey} className="absolute flex min-w-[28px] -translate-x-1/2 flex-col items-center text-center" style={{ left: `${percentForDate(date)}%` }}>
                  <span className={cx("mb-1.5 rounded-full transition-all", isHighlighted ? "h-4 w-[2px] bg-gradient-to-b from-violet-400 to-sky-400 shadow-[0_0_10px_rgba(168,85,247,.45)]" : cx("h-2.5 w-px", isDarkTimeline ? "bg-slate-500/70" : "bg-slate-300/90"))} />
                  <span className={cx("text-[10px] font-black leading-none transition-all", isHighlighted ? (isDarkTimeline ? "text-violet-100" : "text-violet-700") : t.textSoft)}>{date.getDate()}</span>
                </div>
              );
            })}
          </div>
    ...
    new Map(
      tasks
        .map((task) => {
          ...
          const snappedDate = snapDateToAxisTicks(rawDue, axisTicks);
          if (!snappedDate) return null;
          const label = selectedTimelineView === "year"
            ? formatDate(snappedDate, { month: "short", year: "numeric" })
            : formatDate(snappedDate, { day: "2-digit", month: "short" });
          return { key: snappedDate, label };
        })
        .filter(Boolean)
        .map((item) => [item.key, item])
    ).values()
  );
  const highlightedAxisDates = new Set(highlightedAxisItems.map((item) => item.key));

  function previewDraggedTaskDate(taskId, clientX) {
    const rect = chartRef.current?.getBoundingClientRect();
    const rawDate = dateFromTimelineDragDelta(dragMetaRef.current.startX, clientX, rect, dragMetaRef.current.originalDate, visibleRangeStart, visibleRangeMs);
    const nextDate = snapDateToAxisTicks(parseLocalDate(rawDate), axisTicks);
                ...
                <div className={cx("rounded-[1.75rem] border p-4", t.cardSolid)}>
                  ...
                  >
                      ...
                      {tasks.map((task, index) => {
                        ...
                        const snappedDueDate = dueIsVisible ? (snapDateToAxisTicks(rawDue, axisTicks) || visibleDueDate) : visibleDueDate;
                        const due = parseLocalDate(snappedDueDate);
                        const column = columns.find((item) => item.id === task.columnId);
                        const barClass = column?.accent || "from-slate-400 to-slate-600";
                        const taskProgress = progressOf(task);
                        ...
                        return (
                          <React.Fragment key={task.id}>
                            ...
                            <span
                              ...
                              title={`Marker: ${formatDate(snappedDueDate, { day: "2-digit", month: "2-digit", year: "numeric" })}`}
                            />
                            <button
                              type="button"
                              ...
                              title={`${task.title} — termin: ${formatDate(snappedDueDate, { day: "2-digit", month: "2-digit", year: "numeric" })}`}
                            >
                              ...
                              <span className="block truncate text-[10px] font-bold opacity-85">
                                {column?.title || "Bez etapu"} • {taskProgress}% • {formatDate(snappedDueDate, { day: "2-digit", month: "2-digit" })}
                              </span>
                              {isDragging && (
                                <span className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-slate-950 shadow-lg">
                                  {formatDate(snappedDueDate, { day: "2-digit", month: "long" })}
                                </span>
                              )}
                            </button>
                          </React.Fragment>
...
function Column({
  ...
  function handleDrop(event) {
    event.preventDefault();
    if (draggedTaskId) onMoveTask(draggedTaskId, column.id);
  ...
  return (
    <motion.article
      ...
      onDrop={handleDrop}
      className={cx(
        "min-w-0 rounded-[1.6rem] border p-2.5 shadow-xl backdrop-blur-xl transition lg:rounded-[1.9rem] lg:p-4",
        isOver
          ? isDark
    ...
    >
      ...
      <div className="grid gap-2.5 lg:gap-3">
        ...
        {tasks.length === 0 && (
          <div className={cx("rounded-3xl border border-dashed p-6 text-center text-sm", t.border, t.textMuted)}>
            W tej kolumnie nie ma jeszcze zadań.
          </div>
        )}
      </div>
    </motion.article>
...
function TaskCard({ t, isDark, task, onOpen, onToggleSubtask, onDragStart, onDragEnd }) {
  ...
  const hiddenSubtasks = Math.max(0, total - visibleSubtasks.length);
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="line-clamp-2 text-sm font-black leading-snug lg:text-base">{task.title}</h3>
          {images.length > 0 && (
      ...
      {task.description && (
        <p data-task-description className={cx("mb-3 line-clamp-3 text-sm leading-6", t.textMuted)}>
          {task.description}
        </p>
      )}
          ...
          <div className="grid gap-1.5">
            {visibleSubtasks.map((item) => (
              ...
              >
                ...
                <span className={cx("line-clamp-1", item.done && "text-slate-400 line-through")}>{item.text}</span>
              </button>
            ))}

            {hiddenSubtasks > 0 && (
              <p className={cx("pl-6 text-xs font-semibold", t.textSoft)}>+{hiddenSubtasks} więcej</p>
            )}
          </div>
